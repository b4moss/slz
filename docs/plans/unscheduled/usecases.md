...
- `slz git gone` — remote 側で消えた tracking branch をまとめて消す（確認必須）
- `slz git wip` / `slz git fire` — ローカル一時 commit と、push まで含む緊急退避は分ける
- `slz gh pr` — 今の branch の PR をブラウザで開くに閉じるなら採用可
